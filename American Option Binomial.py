...
for i in range(0, 7):
    for j in range(0, num):
        def eur_c(vol, r, f, k, t):
            ...
            return call
        eur_call = (lambda vol: eur_c(vol,
                                      r,
                                      price[i],
                                      strike_call[i * num + j],
                                      T + float(i) / 365) - price_call[i * num + j])
...
for k in range(0, 7):
    for j in range(0, num):
        ...
        for i in xrange(N - 1, -1, -1):
            fs_a[:-1] = np.exp(-r * deltaT) * \
                (p * fs_a[1:] + oneMinusP * fs_a[:-1])
            fs2[:] = fs2[:] / d
            fs_a[:] = np.maximum(fs_a[:], fs2[:] - fs3[:])
            fs_e[:-1] = np.exp(-r * deltaT) * \
                (p * fs_e[1:] + oneMinusP * fs_e[:-1])
